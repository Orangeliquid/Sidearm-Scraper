...
def calcuate_pers(players: list[Player]): 
    # Get League Averages for only GMAC players
    lg_AST, lg_FGM, lg_FGA, lg_PTS, lg_FTM, lg_FTA, lg_OREB, lg_REB, lg_TOV, lg_PF = calculate_league_avgs([player for player in players if player.CONFERENCE])

    # Get Team Stats (AST, FGM) for all teams
    team_ast_dict, team_fgm_dict = calculate_team_stats(players)

    # Calulate factor, VOP and DRB%
    ...
    for player in players:
        calculate_uPER(player, factor, VOP, DRB_percent,
                        team_ast_dict.get(player.TEAM), team_fgm_dict.get(player.TEAM),
                        lg_FTM, lg_FTA, lg_PF)
...
def calculate_uPER(player: Player, factor: float, VOP: float, DRB_percent: float, team_ast, team_fgm, lg_ftm, lg_fta, lg_pf):
